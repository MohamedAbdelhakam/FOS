...
void page_fault_handler(struct Env * curenv, uint32 fault_va)
{
	//TODO: [PROJECT 2025 - MS1 + MS2 - [2] Page Fault Handler]
	// Write your code here, remove the panic and write your code
	//panic("page_fault_handler() is not implemented yet...!!");

	//refer to the project documentation for the detailed steps
	//TODO: [PROJECT 2025 - BONUS5] Change WS Size according to “Program Priority”
	    ...
	    if (env_page_ws_get_size(curenv) < curenv->page_WS_max_size)
			{
				...
				map_frame(curenv->env_page_directory, frame_info, (void *)fault_va, PERM_USER | PERM_WRITEABLE | PERM_PRESENT);
				...
				if (ret == E_PAGE_NOT_EXIST_IN_PF)
				{
					if (fault_va >= USTACKBOTTOM && fault_va < USTACKTOP)
					{
						...
						{
							panic("no space on page file");
						}
					}
					else
					{
						panic("worng memory access");
					}
				}
				...
				env_page_ws_set_entry(curenv, ws_index, fault_va);
	    ...
	    else
	    {
	        ...
	        {
	            victI = 0;
	            victi_va = env_page_ws_get_virtual_address(curenv, 0);
	            uint32 min_time = env_page_ws_get_time_stamp(curenv, 0);

	            for (uint32 i = 1; i < curenv->page_WS_max_size; i++)
	            {
	                uint32 va = env_page_ws_get_virtual_address(curenv, i);
	                uint32 ts = env_page_ws_get_time_stamp(curenv, i);
	                if (ts < min_time)
	                {
	                    min_time = ts;
	                    victi_va = va;
	          ...
	          uint32 va = env_page_ws_get_virtual_address(curenv, index);
	            uint32 perm = pt_get_page_permissions(curenv, va);
	                ...
	                if ((perm & PERM_USED) == 0)
	             {
	                ...
	                index = (index + 1) % ws_size;
	     }
	        }
	        uint32 *ptptr = NULL;
	    struct Frame_Info *victim_frame = get_frame_info(curenv->env_page_directory,
	                                        (void *)victi_va, &ptptr);
	       uint32 perm = pt_get_page_permissions(curenv, victi_va);
	        if (perm & PERM_MODIFIED)
	        {
	            int r = pf_update_env_page(curenv, (void *)victi_va, victim_frame);
	            if (r == E_PAGE_NOT_EXIST_IN_PF)
	            {
	                if (pf_add_empty_env_page(curenv, victi_va, 0) == E_NO_PAGE_FILE_SPACE)
	                    panic("no space in page file");
	                pf_update_env_page(curenv, (void *)victi_va, victim_frame);
	            }
	        }
	        unmap_frame(curenv->env_page_directory, (void *)victi_va);
	        env_page_ws_clear_entry(curenv, victI);

	        map_frame(curenv->env_page_directory, victim_frame,
	                  (void *)fault_va, PERM_USER | PERM_WRITEABLE | PERM_PRESENT);
	        int r = pf_read_env_page(curenv, (void *)fault_va);
	        if (r == E_PAGE_NOT_EXIST_IN_PF)
	        {
	            if (fault_va >= USTACKBOTTOM && fault_va < USTACKTOP)
	            {
	            ...
	            {
	                panic("page not found and not stack page");
	            }
	        }

	        env_page_ws_set_entry(curenv, victI, fault_va);
	        curenv->page_last_WS_index = (victI + 1) % curenv->page_WS_max_size;
	    }
