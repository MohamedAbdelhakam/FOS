...
{
	uint32 *ptr_PT;
	ptr_PT=kmalloc(PAGE_SIZE);

	if(ptr_PT==NULL)
	{
		panic("not exist complete space in heap");
	}

	ptr_page_directory[PDX(virtual_address)]=CONSTRUCT_ENTRY(kheap_physical_address((unsigned int)ptr_PT),(PERM_PRESENT|PERM_USER|PERM_WRITEABLE));
	memset(ptr_PT,0,PAGE_SIZE);
	tlbflush();

	return ptr_PT;
	//cprintf("---------------- create P.T ---------------\n");
	//cprintf("** ptr_page_directory %x VA %x\n",ptr_page_directory, virtual_address);
	//cprintf("** create new table\n");
	//cprintf("** create table for VA %x\n", virtual_address);
	//cprintf("-- create table for VA  %x \n", virtual_address);
}
...
{
	//TODO: [PROJECT 2025 - MS2 - [4] User Heap] allocateMem() [Kernel Side]
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	for(VA=ROUNDDOWN(virtual_address,PAGE_SIZE); VA<(virtual_address+size); VA+=PAGE_SIZE)
	{
		pf_remove_env_page(e,VA);
		get_page_table(e->env_page_directory,(void*)VA,&ptr_PT);

		if((ptr_PT!=NULL) && (ptr_PT[PTX(VA)]&PERM_PRESENT))
		{
			pt_set_page_permissions(e,VA,0,PERM_AVAILABLE);
		...
		{
			if((ptr_PT!=NULL) && (ptr_PT[k]&PERM_PRESENT))
			{
				f_emp=0;
				break;
			}
...
{
	//TODO: [PROJECT 2025 - BONUS3] User Heap Realloc [Kernel Side]
	//your code is here, remove the panic and write your code
